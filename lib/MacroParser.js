...
/**
 * Append suffix and replace an older file.
 * Template-Files use file-names with *.tmpl suffix.
 * 
 * @returns {String}
 */
 ...
 * @returns {___cmd0}
 */
MacroPreProcessor.prototype.extractCmd = function(
		startInstrPos, endInstrPos) {
	var txt = data.substring(startInstrPos, endInstrPos);
...
MacroPreProcessor.prototype.preProcessContent = function(data, tmplPath) {
	...
	// loop over
	for ( var startInstrPos = data.indexOf(KEYWORD_START_CMD_SCOPE, lastOffset); 
	             (startInstrPos > -1); 
	          startInstrPos = data.indexOf(KEYWORD_START_CMD_SCOPE, lastOffset)) {
		console.log( "-- found command-block. start-pos: " + startInstrPos );
		
		if( startInstrPos > lastOffset ) {
			pipe( lastOffset, startInstrPos );
			console.log( "-- write content before this block to output-write-stream" );
		}
		...
		if( endInstrPos < 0 ) {
			...
			var cmd = this.extractCmd( startInstrPos, endInstrPos );
			
			if( cmd ) {
				var validated = cmd.validate();
				console.log("-- command not found. skip command: %j ", validated  );
	...
	try {
		...
		console.log( "-- skip closing file-write-stream: %j", j );
	};
